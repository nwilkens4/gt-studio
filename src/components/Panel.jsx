...
export default function Panel({ children }) {
  ...
  return (
    <div className="flex h-full flex-shrink-0">
      {/* Toggle tab — z-index ensures it stays above the Three.js canvas */}
      <button
        onClick={() => setOpen((v) => !v)}
        aria-label={open ? 'Collapse panel' : 'Expand panel'}
        style={{ position: 'relative', zIndex: 1, background: 'rgba(255,255,255,0.02)', borderLeft: '1px solid rgba(255,255,255,0.07)' }}
      ...
      {/* Sliding content */}
      <div className="h-full overflow-hidden" style={{ width: open ? '272px' : '0px', transition: 'width 0.28s cubic-bezier(0.16,1,0.3,1)' }}>
        <div
